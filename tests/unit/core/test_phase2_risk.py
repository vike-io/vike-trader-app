...
def test_order_target_percent_reaches_notional():
    ...
    eng = SingleSymbolEngine(_flat_bars(prices), S(), cash=10_000.0)
    eng.run()
...
def test_leverage_cap_limits_position():
    ...
    prices = [100.0, 100.0, 100.0]
    eng = SingleSymbolEngine(_flat_bars(prices), S(), cash=10_000.0, leverage=3.0)
    eng.run()
...
def test_liquidation_force_closes_on_crash():
    ...
    eng = SingleSymbolEngine(bars, S(), cash=1_000.0, leverage=10.0, maint_margin=0.05)
    eng.run()
