...
def fetch_chain(provider_names, symbol, interval, start_ms, end_ms, progress=None,
                select=select_source, settings_by_provider=None):
    ...
    for name in provider_names:
        try:
            ...
            src = select(symbol, provider=name, settings=settings)
...
def fetch_for(symbol, interval, start_ms, end_ms, *, root, linked_provider=None,
    ...
    Reads per-provider settings from the persisted config and forwards them to ``fetch_chain``
    so that base_url/pause/api_key overrides are applied transparently at fetch time.
    """
