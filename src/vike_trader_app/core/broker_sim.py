...
cannot call back into Python cheaply) and is pinned to them by ``tests/test_reconciliation.py``.
Change a cost rule HERE and update the kernel's mirror in the same commit, or the
...
def fee(size: float, price: float, rate: float, multiplier: float) -> float:
    """Transaction fee = ``rate`` on the (multiplier-scaled) notional. Always >= 0."""
    return size * price * rate * multiplier


def funding_charge(pos_size: float, mark_price: float, funding_rate: float, multiplier: float) -> float:
