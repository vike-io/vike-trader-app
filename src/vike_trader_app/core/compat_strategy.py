"""Compat shim — the original single-symbol Strategy API, preserved verbatim.

The class was renamed ``SingleSymbolStrategy`` to free the name ``Strategy``
for the new unified (multi-symbol) API in Task 3.  ``core.strategy`` re-exports
``SingleSymbolStrategy as Strategy`` so all existing imports and subclasses
continue to work without any change.

.. deprecated::
    Subclass ``SingleSymbolStrategy`` (or keep importing ``Strategy`` from
    ``core.strategy`` while the alias lives) until the unified Strategy lands.
"""
...
def _history_executor() -> concurrent.futures.ThreadPoolExecutor:
    """Lazy shared pool for SingleSymbolStrategy.history_async (daemon workers; reads are thread-safe, #259)."""
...
class SingleSymbolStrategy:
    ...
    @classmethod
    def make(cls, **params) -> "SingleSymbolStrategy":
