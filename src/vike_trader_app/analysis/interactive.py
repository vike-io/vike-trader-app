...
def write_interactive_html(path, result, title: str = "Backtest", timestamps=None, inline: bool = True) -> Path:
    ...
    fig.add_trace(go.Scatter(x=x, y=eq, name="equity", line={"color": "#ff6a00"}), row=1, col=1)
    fig.add_trace(go.Scatter(x=x, y=dd, name="drawdown", fill="tozeroy", line={"color": "#f85149"}), row=2, col=1)
    fig.update_layout(
        title=f"{title}  ·  return {metrics.total_return(eq) * 100:.2f}%  ·  Sharpe {metrics.sharpe(eq):.2f}",
        template="plotly_dark", paper_bgcolor="#0d1117", plot_bgcolor="#0d1117",
        font={"family": "JetBrains Mono, monospace", "color": "#c9d1d9"}, showlegend=False,
    )
    p = Path(path)
